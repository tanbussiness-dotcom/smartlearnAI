...
`;
  ...
  try {
    ...
    try {
      validatedOutput = ValidateLessonOutputSchema.safeParse(safe);
    } catch (zerr: any) {
      console.warn('[validateLesson] Zod threw exception:', zerr?.message || zerr);
      return {
        valid: false,
    ...
    if (!validatedOutput.success) {
      console.warn('[validateLesson] Schema mismatch:', validatedOutput.error?.errors);
      return {
        valid: false,
        confidence_score: safe.confidence_score,
        issues:
          safe.issues.length > 0
            ? safe.issues
            : [{ type: 'SchemaMismatch', detail: 'Validation failed at schema level' }],
      };
    }
