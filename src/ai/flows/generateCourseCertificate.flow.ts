 ...
 *
 * This function checks a user's roadmap progress, generates a PDF certificate
...
export async function generateCourseCertificate(input: GenerateCourseCertificateInput): Promise<GenerateCourseCertificateOutput> {
  ...
  try {
    ...
    if (useAIText) {
      try {
        const aiPrompt = `Write a short, inspiring, two-sentence congratulatory message for a certificate. The student, ${userName}, has just completed the course "${courseTitle}". The tone should be formal and celebratory.`;
        const aiText = await generateWithGemini(aiPrompt, false);
        if (aiText) {
          certificateBody = aiText;
        }
      } catch (e) {
