...
export async function generateLesson(input: z.infer<typeof GenerateLessonInputSchema>): Promise<z.infer<typeof GenerateLessonOutputSchema>> {
  ...
  try {
    ...
    if (!lessonDraft || !lessonDraft.content || typeof lessonDraft.content !== 'string' || lessonDraft.content.trim().length < 100) {
    ...
    // CRITICAL FIX: Check for empty object and validity
    if (!validationResult || Object.keys(validationResult).length === 0 || !validationResult.valid) {
      const issues = validationResult?.issues ? JSON.stringify(validationResult.issues) : "AI failed to return a valid validation object.";
